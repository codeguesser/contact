...
+(CGDataResult *)getResultFromDic:(NSDictionary *)dic className:(NSString *)classname{
    ...
    if (dic&&[[dic objectForKey:API_STATUS] isKindOfClass:[NSString class]]) {
        result.errorMsg = [dic objectForKey:API_MSG];
        result.status =[dic objectForKey:API_STATUS];
        id datalist = [NSClassFromString(classname) getObjecsFromDic:[dic objectForKey:API_CONTENT]];
        if (datalist) {
